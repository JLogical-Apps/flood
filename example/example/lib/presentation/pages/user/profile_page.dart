...
class ProfilePage with IsAppPageWrapper<ProfileRoute> {
  @override
  AppPage<ProfileRoute> get appPage => AppPage<ProfileRoute>(
        builder: (context, route) {
          final loggedInUserId = useLoggedInUserId();
          final loggedInUserModel = useEntityOrNull<UserEntity>(loggedInUserId);
          final budgetsModel =
              useQuery(Query.from<BudgetEntity>().where(Budget.ownerField).isEqualTo(loggedInUserId).all());
                                ...
                                ActionItem(
                                  ...
                                  onPerform: (context) async {
                                    await context.showStyledDialog(StyledPortDialog(
                                      ...
                                      port:
                                          (Budget()..ownerProperty.set(loggedInUserId)).asPort(context.corePondContext),
