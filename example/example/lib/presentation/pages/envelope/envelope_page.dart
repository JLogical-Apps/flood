...
class EnvelopePage with IsAppPageWrapper<EnvelopeRoute> {
  @override
  AppPage<EnvelopeRoute> get appPage => AppPage(
        builder: (context, EnvelopeRoute route) {
          final envelopeModel = useEntityOrNull<EnvelopeEntity>(route.idProperty.value);
          final envelopeEntity = envelopeModel.getOrNull();

          final envelopeTransactionsModel = useQuery(
              BudgetTransactionEntity.getEnvelopeTransactionsQuery(envelopeId: route.idProperty.value).paginate());

          final trayModel = useQueryOrNull(envelopeEntity?.value.trayProperty.value
              ?.mapIfNonNull((trayId) => Query.getByIdOrNull<TrayEntity>(trayId)));
                ...
                body: StyledList.column.centered.withScrollbar(
                  children: [
                    ...
                    PaginatedQueryModelBuilder(
                      ...
                      builder:
                          (List<BudgetTransactionEntity> envelopeTransactionEntities, Future Function()? loadNext) {
                          ...
                          children: envelopeTransactionEntities
                              .map((entity) => TransactionCard(
                                    ...
                                    transactionViewContext:
                                        TransactionViewContext.envelope(envelopeId: envelopeEntity.id!),
                                    actions: [
                                      ActionItem(
                                        titleText: 'Delete',
      ...
      ).withParent((context, route) async {
