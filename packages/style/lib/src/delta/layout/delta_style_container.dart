...
class DeltaStyleContainerRenderer with IsTypedStyleRenderer<StyledContainer> {
  @override
  ...
  ColorPalette getBackgroundColor(BuildContext context, {required StyledContainer container}) {
    final deltaStyle = context.style() as DeltaStyle;

    final color = container.color;
    if (color != null) {
      if (color.opacity < 1) {
        final mixedColor = context.colorPalette().baseBackground.mix(color, (color.opacity * 100).round());
        return deltaStyle.getColorPaletteFromBackground(mixedColor);
      }
      return deltaStyle.getColorPaletteFromBackground(color);
    }
